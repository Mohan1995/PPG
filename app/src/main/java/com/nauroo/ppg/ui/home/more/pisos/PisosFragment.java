package com.nauroo.ppg.ui.home.more.pisos;

import android.content.pm.ActivityInfo;
import android.os.Bundle;

import androidx.fragment.app.Fragment;

import android.view.LayoutInflater;
import android.view.View;
import android.view.ViewGroup;

import com.nauroo.ppg.R;
import com.nauroo.ppg.databinding.FragmentPisos2Binding;
import com.nauroo.ppg.ui.home.HomeActivity;

/**
 * A simple {@link Fragment} subclass.
 * Use the {@link PisosFragment#newInstance} factory method to
 * create an instance of this fragment.
 */
public class PisosFragment extends Fragment {

    // TODO: Rename parameter arguments, choose names that match
    // the fragment initialization parameters, e.g. ARG_ITEM_NUMBER
    private static final String ARG_PARAM1 = "param1";
    private static final String ARG_PARAM2 = "param2";

    // TODO: Rename and change types of parameters
    private String mParam1;
    private String mParam2;

    FragmentPisos2Binding binding;
    public PisosFragment() {
        // Required empty public constructor
    }

    /**
     * Use this factory method to create a new instance of
     * this fragment using the provided parameters.
     *
     * @param param1 Parameter 1.
     * @param param2 Parameter 2.
     * @return A new instance of fragment PisosFragment.
     */
    // TODO: Rename and change types and number of parameters
    public static PisosFragment newInstance(String param1, String param2) {
        PisosFragment fragment = new PisosFragment();
        Bundle args = new Bundle();
        args.putString(ARG_PARAM1, param1);
        args.putString(ARG_PARAM2, param2);
        fragment.setArguments(args);
        return fragment;
    }

    @Override
    public void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        if (getArguments() != null) {
            mParam1 = getArguments().getString(ARG_PARAM1);
            mParam2 = getArguments().getString(ARG_PARAM2);
        }
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        // Inflate the layout for this fragment
        binding=FragmentPisos2Binding.inflate(inflater,container,false);

        //Force app to stick with Portrait Mode
        getActivity().setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_PORTRAIT);


        binding.pisosCalculatorLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                PisosCalculator pisosCalculator=new PisosCalculator();
                HomeActivity activity = (HomeActivity) getContext();
                activity.addFragmentWithoutToolbar(pisosCalculator,"","");
            }
        });

        binding.systemSelectorLayout.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {

            }
        });
        return binding.getRoot();
    }

    @Override
    public void onDestroy() {
        //Reset Orientation to normal
        getActivity().setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);

        super.onDestroy();

    }
}